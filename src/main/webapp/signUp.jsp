...
<body>
	...
	<table>
			...
			<tr>
				<td><label><fmt:message key = "key.place" bundle = "${lang}" /></label></td>
				<td>
				<select name="place">
					<c:forEach items="${listPlace}" var="listPlace">
					<option value="${listPlace.id}">${listPlace.name}</option>
					</c:forEach>
				</select>
				</td>
			</tr>
			...
			<tr>
			<td></td><td><input type="submit" name="addTransaction" value=<fmt:message key = "key.transactionAdd" />></td>
			</tr>
